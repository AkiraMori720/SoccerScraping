...
EOD;
                ...
                foreach ($leagues as $league) {
                    ...
                    printMessage("   - Checking clubs for [{$league['site']}][{$league['country']}][{$league['league']}] ...", "");
                    ...
                    try {
                        ...
                        if($jsonData != null) {
                            $totalFound = sizeof($jsonData);
                            printMessage("     Found {$totalFound} club(s).", "");
                            if($totalFound > 0) {
    ...
    // Get Referees
    ////////////////////////////////////////////////////////////////////////////////////
    printMessage("=> Retrieving Referees...", "");
    ...
    foreach ($recBaseCountries as $recCountry) {
        ...
        // Fetch refers
        foreach ($leagues as $league) {
            printMessage("   - Checking for the country [{$selectedCountry}][{$league['league']}] ...", "");
            try {
                ...
                if($jsonData != null) {
                    $totalFound = sizeof($jsonData);
                    printMessage("     Found {$totalFound} referee(s).", "");
                    if($totalFound > 0) {
    ...
    // Get Matches
    ////////////////////////////////////////////////////////////////////////////////////
    printMessage("=> Retrieving Matches...", "");
    ...
    foreach ($datesToCheck as $matchDate) {
        try {
            foreach ($sites as $siteName => $command) {
                printMessage("   - Checking for [{$matchDate}][{$siteName}] ...", "");
                ...
                if($jsonData != null) {
                    $totalFound = sizeof($jsonData);
                    printMessage("     Found {$totalFound} match(es).", "");
                    if($totalFound > 0) {
            ...
            printMessage("    Failed to fetch matches! " . $e->getMessage(), "");
        }
    }
    ...
    // Get Tips
    ////////////////////////////////////////////////////////////////////////////////////
    printMessage("=> Retrieving Tips for matches...", "");
    ...
    try {
        ...
        $divsInTips = array();
        // Predictz
        printMessage("   - Checking on Predictz ...", "");
        ...
        // WinDrawWin
        printMessage("   - Checking on Windrawwin ...", "");
//            $dateForWindrawwin = LIVE_SERVER ? $_gActiveDate_ : convertTimeZoneOfDate($_gActiveDate_, 'Europe/Madrid', 'Asia/Shanghai');
        ...
        // SoccerWay
        printMessage("   - Checking on SoccerWay ...", "");
